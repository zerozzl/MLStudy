...
# 生成模型
def model_create(SparkMaster, TrainData, inputsize, outputsize, alpha, numepochs, batchsize, ModelFile, ErrPngFile):
    conf = SparkConf().setAppName('MNIST CNN Train').setMaster(SparkMaster);
    sc = SparkContext(conf=conf);

    trainData = loadSparkData(sc, TrainData);
    trainData.cache();
...
# 测试模型  
def model_test(SparkMaster, TestData, ModelFile, inputsize, outputsize):
    conf = SparkConf().setAppName('MNIST CNN Test').setMaster(SparkMaster);
    sc = SparkContext(conf=conf);
    
    testData = loadSparkData(sc, TestData);
...
def main():
    ...
    HDFSFolder = 'hdfs://localhost:9000/datas/mnist/';
    TrainData = HDFSFolder + 'train-datas.csv';
    TestData = HDFSFolder + 'test-datas.csv';
    
    resultFolder = '/home/hadoop/ProgramDatas/MLStudy/MNIST/Spark/';
    ...
    model_create(SparkMaster, TrainData, inputsize, outputsize, alpha, numepochs, batchsize, ModelFile, ErrPngFile);
