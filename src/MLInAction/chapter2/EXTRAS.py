...
def createDatingDist(folder):
    n = 1000  # number of points to create
    xcord = numpy.zeros((n))
    ycord = numpy.zeros((n))
    markers = []
    colors = []
    fw = open(folder + '/testSet.txt', 'w')
    for i in range(n):
        [r0, r1] = numpy.random.standard_normal(2)
        myClass = numpy.random.uniform(0, 1)
        if (myClass <= 0.16):
            fFlyer = numpy.random.uniform(22000, 60000)
            tats = 3 + 1.6 * r1
            markers.append(20)
            colors.append(2.1)
        ...
        fw.write("%d\t%f\t%f\t%d\n" % (fFlyer, tats, numpy.random.uniform(0.0, 1.7), classLabel))
